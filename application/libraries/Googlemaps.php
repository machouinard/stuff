...
//	var $api_key                            = 'AIzaSyBzcpy3cp1iGCTWnjgY3O0oRHcwrUccDGA';    //SET GOOGLE MAPS API KEY HERE  / ADDED BY Mark Chouinard default key is for stuff.chouinard.me
	var $api_key                            = '';    //SET GOOGLE MAPS API KEY HERE  / ADDED BY Mark Chouinard default key is for stuff.chouinard.me
	...
	var $custom_listener                    = FALSE;                    //ADD CUSTOM EVENT LISTENER HERE  used for displaying distance from center to click point  CHOUINARD
	...
	function create_map()
	{
		...
		$apiLocation .= '.google.com/maps/api/js?key='.$this->api_key.'&sensor='.$this->sensor.'&libraries='.$this->libraries;
		...
		$this->output_js_contents .= '
			var '.$this->map_name.'; // Global declaration of the map
                        var markersArray = []; ////Chouinard
                        var polyLineArray= [];  ////Chouinard
		...
		if ($this->bicyclingOverlay) {
			$this->output_js_contents .= 'var bikeLayer = new google.maps.BicyclingLayer();
                ...
                //MY DISTANCE EVENT LISTENER chouinard
                if($this->custom_listener){
                $this->output_js_contents .= $this->custom_listener;
                }
		...
		//MEASURE DISTANCE chouinard
                $this->output_js_contents .= "
                function measureDistance(location) {
                        if(markersArray.length > 0){
                            for(var x = 0; x < markersArray.length; x++){
                                markersArray[x].setMap(null);
                            }
                        }
                        var path = poly.getPath();  // from trying to add polyline  //poly
                        if(path.length >1){
                            path.pop(1);
                        }  //poly
                        ...
                        position: location,
                        title: '#' + path.getLength(),  ///from trying to add polyline
                        map: $this->map_name
                        });
                        markersArray.push(marker);

                        var current = new google.maps.LatLng($this->center);

                        var distance = google.maps.geometry.spherical.computeDistanceBetween(current, location);
                        var feet = Math.ceil(distance*3);
                        feet = convertToMiles(feet);
                        distanceDiv = document.getElementById('distance');
                        distanceDiv.innerHTML = 'That\'s ' + feet + ' away';
                ...
                ///////CONVERT TO MILES
                $this->output_js_contents .= "
                  function convertToMiles(nStr)
                        {
                            var miles = nStr/5280;
                            if(miles > .5){
                                miles = Math.round(miles*Math.pow(10,2))/Math.pow(10,2);  //trying to round to 2 decimal places
                                miles = addCommas(miles);
                                return miles + ' miles';
                            }else{
                                var feet = addCommas(nStr);
                                return feet + ' feet';
                            }
                        }
                ";
		///////// ADD COMMAS
                ...
                function showOverlays() {
                    if (markersArray) {
                        ...
                        markersArray[i].setMap($this->map_name);
                        }
                    }
                }
